...
export default function PostBody({post, style = {}}: {style?: any, post: Post}) {
  ...
  let __html = sanitized
    ...
    .replace(BACKLINK_HTML_SAFE_REGEX, `<a class="text-blue-600 visited:text-purple-600 hover:text-blue-500" href="#${post.board ? Router.board(post.board) : ""}/${post.thread ? `${post.thread.n}/` : ""}$1">&gt;&gt;$1$2</a>`) // Post backlinks // @HACK
