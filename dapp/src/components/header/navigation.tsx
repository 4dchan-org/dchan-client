            ...
            <span className="text-black text-opacity-50">[
                {!!boards && boards.map(board => (
                    <span className="dchan-navigation-board" key={board.id}>
                        <Link className="text-blue-600 visited:text-purple-600 hover:text-blue-500 hover:text-opacity-100" title={board.title} to={`/${board.name}/${board.id}`}>{board.name}</Link>
                    </span>
                ))}
